...
public:
    ...
    inline size_t write_memory(T* address, T array[], int element_count)
    {
        if (!process_handle)
            return 0;

        size_t bytes_written = 0;
        WriteProcessMemory(process_handle, address, array, sizeof(T) * element_count, reinterpret_cast<SIZE_T*>(&bytes_written));
        return bytes_written;
    }
private:
