...
LPCSTR driverPath = "\\SystemRoot\\System32\\drivers\\FpacDrv.sys";
std::wstring registryPath = L"System\\CurrentControlSet\\Services\\fairplaydriver";
...
bool Driver::LoadDriver()
{
	...
	if (!PathFileExistsA("C:\\Windows\\System32\\drivers\\FpacDrv.sys"))
	{
	...
	{
		std::cout << "[+] Driver já esta carregado..." << std::endl;
		return true;
	}

	std::wstring loadRegPath = L"\\Registry\\Machine\\System\\CurrentControlSet\\Services\\fairplaydriver";
	...
	Sleep(1000);
	std::cout << "[+] Driver Carregado em Kernel!" << std::endl;
...
bool Driver::IsDriverAlreadyLoaded()
{
	...
	if (NT_SUCCESS(oNtQuerySystemInformation((SYSTEM_INFORMATION_CLASS)11, Modules, size, &size)))
	{
		...
		{
			if (strstr((char*)Modules->Modules[i].FullPathName, "FpacDrv.sys"))
			{
...
bool Driver::Cleanup()
{
	...
	if (NT_SUCCESS(oNtUnloadDriver(&regPath)))
		std::cout << "[+] Driver Fechado" << std::endl;;
...
BOOL DriverObject::isConnected()
{
	...
	if (DeviceIoControl(this->hDriver, IO_STARTUPREQUEST, &Request, sizeof(Request), &Request, sizeof(Request), NULL, NULL))
	{
		if (Request.Buffer == 4)
		{
			std::cout << "[+] Conexao estabilizada com o kernel..." << std::endl;
			return TRUE;
		}
		else return FALSE;
	}
...
BOOL DriverObject::protectProcesses(ULONG ProcessIDS[2])
{
	...
	if (DeviceIoControl(this->hDriver, IO_PROTECTEDPROCESSINFO, &Request, sizeof(Request), &Request, sizeof(Request), NULL, NULL))
	{
		if (Request.Buffer == 1)
		{
			...
			std::cout << "[+] Processo protegido pelo driver" << std::endl;
			return TRUE;
		}
		else return FALSE;
	}
